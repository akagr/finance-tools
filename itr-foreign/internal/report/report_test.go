...
func sampleReport() *fa.Report {
	...
	amt := fa.Amount{INR: model.NewMoney(model.INR, big.NewRat(120000, 1)), Audit: []fx.Conversion{conv}}
	return &fa.Report{
		Year: 2024,
		A3: []fa.A3Row{{
...
func render(t *testing.T, f Format, r *fa.Report) string {
	t.Helper()
	rnd, err := For(f)
	if err != nil {
		t.Fatal(err)
