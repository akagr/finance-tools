...
See [`docs/itr-foreign-plan.md`](docs/itr-foreign-plan.md) for the research,
...
the XML. Save it under `itr-foreign/private/` (gitignored), then use `--statement`.
...
```sh
# (one-time) build the CLI from the itr-foreign/ directory
go build -o itrforeign ./cmd/itrforeign
...
./itrforeign fetch-prices --year 2026
```
...
```sh
go build ./cmd/itrforeign      # from itr-foreign/
go test ./...
```
